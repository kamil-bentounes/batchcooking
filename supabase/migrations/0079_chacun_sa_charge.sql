-- ═══════════════════════════════════════════════════════════════════════════
-- 0079 · Corriger une charge n'en ouvre plus douze autres
--
-- 0078 fait ouvrir à `corrige_la_charge` tous les mois de `debut` à aujourd'hui,
-- pour qu'il ne reste aucun trou dans le passé. Mais il appelle `ouvre_le_mois`,
-- qui ouvre le mois pour TOUTES les charges du foyer.
--
-- Mesuré, sans jamais feuilleter un mois : on pose la taxe foncière et le
-- loyer, on corrige sa date d'entrée, on saisit le relevé de la taxe
-- (966,68 € d'écart), puis on corrige le LOYER — une charge sans aucun rapport.
-- Neuf mois s'ouvrent pour tout le foyer, le provisionné de la taxe bouge,
-- l'écart déjà posé ne bouge pas, et la garde « une seule fois » interdit de
-- ressaisir.
--
--   total porté sur la taxe foncière : 2 416,64 € pour une facture de 1 450 €.
--
-- C'est 0069 qui revient par une autre porte, et c'est 0078 qui l'a ouverte.
-- ═══════════════════════════════════════════════════════════════════════════

-- ── 1 · Ouvrir UNE charge sur un mois, et elle seule ───────────────────────
create or replace function public.ouvre_la_charge(
  la_charge uuid, foyer uuid, le_mois date)
returns integer
language plpgsql
security definer
...
revoke execute on function public.ouvre_la_charge(uuid, uuid, date) from public, anon;
grant   execute on function public.ouvre_la_charge(uuid, uuid, date) to authenticated, service_role;

-- ── 2 · La correction n'ouvre que sa propre charge ─────────────────────────
...
-- ── 3 · Le trou se ferme à la SOURCE ───────────────────────────────────────
--
-- `corrige_la_charge` n'était qu'un des chemins. `tg_entre_le_refige` et
-- `tg_membre_rejoint_les_communes` bouclent sur les mois qui ONT DÉJÀ une
-- dépense et appellent `refige_pour`, qui ne crée jamais rien. Or reculer sa
-- date d'entrée rend éligibles des mois qui n'en avaient aucune.
--
-- Mesuré : membre arrivé en juin, charge annuelle depuis janvier, puis « en
-- fait je suis là depuis janvier ». Sans feuilleter, l'écart posé vaut
-- 604,19 € ; après avoir feuilleté janvier à mai, 0,04 €. Le même geste, deux
-- résultats, selon ce qu'on a regardé entre-temps.
--
-- Tant que le passé dépend du feuilletage, `provisions_a_venir` sera faux quoi
-- que fasse `corrige_la_charge`. On OUVRE, puis on refige.
...
begin
  ...
  /* ⚠️ D'ABORD OUVRIR. Reculer son entrée rend éligibles des mois qui n'ont
     jamais eu de ligne : les refiger ne suffit pas, il faut les faire naître.
     Sinon ils s'ouvrent plus tard, à la première visite du budget, et le
     relevé annuel se calcule entre-temps sur un trou. */
  for m in
    select d.mois::date
    from generate_series(date_trunc('month', depuis)::date, jusqua,
                         interval '1 month') as d(mois)
    order by 1
  loop
    perform public.ouvre_le_mois(m, new.household_id);
  end loop;

  /* Puis refiger, y compris les mois postérieurs à aujourd'hui s'il y en a. */
  for m in
...
begin
  ...
  /* Même raison : quelqu'un qui arrive avec une date passée rend éligibles des
     mois qui n'ont pas de ligne. On les ouvre avant de refiger. */
  for m in
    select d.mois::date
    from generate_series(date_trunc('month', new.entre_le)::date, jusqua,
                         interval '1 month') as d(mois)
    order by 1
  loop
    perform public.ouvre_le_mois(m, new.household_id);
