-- ═══════════════════════════════════════════════════════════════════════════
-- 0080 · La garde, enfin posée là où elle tient
--
-- Quatre tours de correction sur le même défaut, et chaque fois j'ai déplacé
-- l'APPELANT au lieu de poser la garde :
--
--   0078 · `corrige_la_charge` ouvre les mois manquants → il les ouvre pour
--          TOUTES les charges du foyer → 2 416,64 € pour une facture de 1 450 €
--   0079 · il n'ouvre plus que la sienne → mais les triggers d'arrivée, eux,
--          ouvrent pour tout le foyer → 2 054,15 € par l'autre porte
--
-- Le fond : ni `ouvre_le_mois` ni `ouvre_la_charge` ne regardent
-- `releve_annuel`. Tant que la garde manque, le prochain appelant la rouvrira.
-- Elle est ici, dans les deux, et plus aucun appelant ne peut la contourner.
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ── La trace suit la dépense, pas l'inverse ────────────────────────────────
...
-- ── Une note sur `c.household_id = foyer` dans `ouvre_la_charge` ────────────
--
-- Une revue a relevé que retirer cette clause ne fait rougir aucun test. C'est
-- exact, et ce n'est pas un trou : `tg_meme_foyer` refuse d'insérer une dépense
-- dont la charge appartient à un autre foyer. La clause est une ceinture, pas
-- la bretelle — elle évite de faire lever un trigger pour un cas qu'on sait
-- déjà écarter. Écrire un test qui ne mesurerait que le trigger serait un test
-- pour le chiffre, pas pour la protection.
